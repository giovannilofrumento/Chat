...
var wsUri = "ws://" + document.location.host + "/" + pathArray[1] + "/" +  wsEndpoint;
...
function chat(text) {
  //if (text !== null && text.length > 0 ) {
    //var jsonStr = "{'header':{'id':" + broadcastMsgCode + "}, 'body':{'data':'" + room._username + ": " + text + "'}}";
    var jsonStr = "{'header':{'id':" + broadcastMsgCode + "}, 'body':{'peer':'" + room._username + "','msg':'" + text + "','data'=''}}";
    console.log(jsonStr);
    room._send(jsonStr);
...
/*
function clearChat() {
  $("membroChat").innerHTML = "Non Registrato";
  $('join').className = "";
...
var room = {
  ...
  _onopen: function(){
    ...
    //$('joined').className='';
    //$('phrase').focus();
    var jsonStr = "{'header':{'id':" + joinMemberMsgCode + "}, 'body':{'peer':'" + room._username + "','msg':'','data':''}}";
//    alert(jsonStr);
    room._send(jsonStr);
    ...
    if (typeof m.data === "string") {
      ...
      if (msg.header.id === membersListMsgCode) {
        ...
        if (msg.body.peer !== room._username) {
          document.querySelector('#testo').innerHTML += "<p class='right'>" + msg.body.peer +": " + msg.body.msg + "</p>";
...
/*  
  _showMessage: function(m) {
    if (m) {
      ...
      var c = m.indexOf(':');
      var from = m.substring(0, c);
      var text = m.substring(c + 1);
...
function loggati()
{
  ...
  //document.querySelector('#avatar').innerHTML = '<span id="avatar" title="Fai clic per chiudere la Chat"><a href="#" onclick="chiudi()">f</a></span>';
...
function inizia()
{
  if (document.querySelector('#nome').value !== '')
  {
    ...
    document.querySelector('#nomeUtente').innerHTML = document.querySelector('#nome').value;
...
function scrivi()
{
  if (document.querySelector('#messaggio').value !== '')
  {
    ...
    document.querySelector('#testo').innerHTML += "<p class='left'>" +     document.querySelector('#messaggio').value + "</p>";
