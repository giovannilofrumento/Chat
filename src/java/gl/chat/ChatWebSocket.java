...
public class ChatWebSocket {
  ...
  public void onMessage(String data, Session peer) {
    ...
    if (msg.getHeader().getId() == BROADCAST_MSG_CODE) {
      ...
      for (Session p : PEERS) {
        try {
          //messaggio = "{'header':{'id':" + messageMsgCode + "}, 'body':{'data':'" + msg.getBody().getData() + "'}}";
          // {'header':{'id':4}, 'body':{'data':'ggg: !!!!!'}}
          messaggio = "{'header':{'id':'" + MESSAGE_MSG_CODE + "'}, 'body':{'peer':'" + msg.getBody().getPeer() + "','msg':'" + msg.getBody().getMsg() + "','data':'" + msg.getBody().getData() + "'}}";
